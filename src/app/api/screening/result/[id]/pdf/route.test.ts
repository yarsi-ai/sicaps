...
const mockGenerateResultPdf = vi.mocked(generateResultPdf);
...
function makeRequest(sessionId: string, token?: string): NextRequest {
  const url = token
    ? `http://localhost:3000/api/screening/result/${sessionId}/pdf?token=${token}`
    : `http://localhost:3000/api/screening/result/${sessionId}/pdf`;
...
describe('GET /api/screening/result/:id/pdf', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mockGenerateResultPdf.mockResolvedValue(Buffer.from('%PDF-1.4 fake content'));
  });

  it('returns 200 with PDF binary and correct headers', async () => {
    ...
    expect(response.headers.get('Content-Disposition')).toMatch(
      /^attachment; filename="SICAPS-Screening-\d{4}-\d{2}-\d{2}\.pdf"$/,
    );

    const body = await response.arrayBuffer();
    expect(Buffer.from(body).toString()).toBe('%PDF-1.4 fake content');
  });

  it('calls verifySessionAccess with requireCompleted and passes locale to pdf service', async () => {
    mockVerifySessionAccess.mockResolvedValue({ id: VALID_SESSION_ID, locale: 'en' });
    ...
    expect(mockGenerateResultPdf).toHaveBeenCalledWith(VALID_SESSION_ID, 'en');
  });

  it('returns 400 VALIDATION_ERROR for invalid session ID', async () => {
  ...
  it('returns 403 INVALID_TOKEN for missing token', async () => {
  ...
  it('returns 500 INTERNAL_ERROR for unexpected errors', async () => {
    mockGenerateResultPdf.mockRejectedValue(new Error('PDF generation failed'));
